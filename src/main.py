...
# ]
folder = "\\test\\nyoba"
MATRIX = fun.accessImage('src\\test.jpg')
...
eigenValues = fun.eigenvalue(covMATRIX, 1)
total = sum(eigenValues)
#menyaring nilai eigen
temp = 0
i = 0
# eigenValFilter = []
...
for i in range(len(eigenFaces)):
    plt.imshow(np.array(fun.getCol(eigenFaces, i, False)).reshape(256, 256), cmap='gray')
    plt.show()
# plt.imshow(np.array(fun.getCol(eigenFaces, eigVecEff-1, False)).reshape(256, 256), cmap='gray')
# plt.show()
